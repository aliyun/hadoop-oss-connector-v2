...
public abstract class FileSystemContractBaseTest {
  ...
  public void testMkdirs() throws Exception {
    ...
    assertTrue(fs.mkdirs(testDir), "Created subdir");
  ...
  public void testRenameDirectoryMoveToExistingDirectory() throws Exception {
    ...
    assertFalse(fs.exists(path(src + "/file1")), "Nested file1 exists");
    assertFalse(fs.exists(path(src + "/subdir/file2")), "Nested file2 exists");
    assertTrue(fs.exists(path(dst + "/file1")), "Renamed nested file1 exists");
    assertTrue(fs.exists(path(dst + "/subdir/file2")), "Renamed nested exists");
  }
  ...
  public void testRenameDirectoryAsExistingDirectory() throws Exception {
    ...
    fs.mkdirs(dst);
    rename(src, dst, true, false, true);
    assertTrue(fs.exists(path(dst + "/dir")), "Destination changed");
    assertFalse(fs.exists(path(src + "/file1")), "Nested file1 exists");
    assertFalse(fs.exists(path(src + "/dir/subdir/file2")), "Nested file2 exists");
    assertTrue(fs.exists(path(dst + "/dir/file1")), "Renamed nested file1 exists");
    assertTrue(fs.exists(path(dst + "/dir/subdir/file2")), "Renamed nested exists");
  }
  ...
  /**
   * trying to move a directory onto its parent dir will build
  ...
  public void testMoveDirUnderParent() throws Throwable {
    ...
    assertEquals(fs.exists(testdir), true, "Source exists: " + testdir);
    assertEquals(fs.exists(parent), true, "Destination exists" + parent);
  }
  ...
  private void assertIsFile(Path filename) throws IOException {
    ...
    assertFalse(status.isSymlink(), "File claims to be a symlink " + fileInfo);
    assertFalse(status.isDirectory(), "File claims to be a directory " + fileInfo);
  }
