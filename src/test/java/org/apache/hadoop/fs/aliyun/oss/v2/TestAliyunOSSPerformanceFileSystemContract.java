...
import org.apache.hadoop.fs.aliyun.oss.v2.legency.AliyunOSSCopyFileTask;
...
public class TestAliyunOSSPerformanceFileSystemContract
  ...
  public void testRootDirAlwaysExists() throws Exception {
    ...
    assertTrue(fs.exists(super.path("/")));
  }

  @Test
  public void testRenameRootDirForbidden() throws Exception {
    assertTrue(renameSupported());
    rename(super.path("/"),
  ...
  public void testGetFileStatusInVersioningBucket() throws Exception {
    ...
    AliyunOSSFileSystemStore store = ((AliyunOSSPerformanceFileSystem)this.fs).getStore();
    for (int i = 0; i < 29; ++i) {
  ...
  public void testGetFileStatusFileAndDirectory() throws Exception {
    ...
    assertFalse(this.fs.getFileStatus(filePath).isDirectory(), "Should not be directory");

    Path dirPath = this.path("/test/oss/dir");
    this.fs.mkdirs(dirPath);
    assertTrue(this.fs.getFileStatus(dirPath).isDirectory(), "Should be directory");
    ...
    for (FileStatus fileStatus: fs.listStatus(parentPath)) {
      assertTrue(fileStatus.getModificationTime() > 0L, "file and directory should be new");
    }
  }
  ...
  public void testMkdirsForExistingFile() throws Exception {
    ...
    assertFalse(this.fs.exists(testFile), "File should not exist");
    this.createFile(testFile);
    assertTrue(this.fs.exists(testFile), "File should exist");
    try {
