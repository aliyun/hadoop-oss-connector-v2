...
/**
 * Provides functionality to read oss file one block at a time.
 */
public class RemoteObjectReader implements Closeable {

  private static final Logger LOG = LoggerFactory.getLogger(
      RemoteObjectReader.class);

  /** We read from the underlying input stream in blocks of this size. */
  private static final int READ_BUFFER_SIZE = 64 * 1024;

  /** The Oss file to read. */
  ...
  /**
   * Constructs an instance of {@link RemoteObjectReader}.
   *
   * @param remoteObject The Oss file to read.
   *
   * @throws IllegalArgumentException if remoteObject is null.
   */
  public RemoteObjectReader(RemoteObject remoteObject) {
...
//    int reqSize = (int) Math.min(size, this.remoteObject.size() - offset);
  ...
  private void readOneBlock(ByteBuffer buffer, long offset, int size, ObjectAttributes context, RemoteLogContext remoteLogContext)
    ...
    try {
      ...
      remoteObject.close(getObjectResult, numRemainingBytes);
    }
  }
}
