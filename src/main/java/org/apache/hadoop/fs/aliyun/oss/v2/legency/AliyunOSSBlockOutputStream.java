...
import org.apache.hadoop.fs.aliyun.oss.v2.model.PartETagParam;
...
public class AliyunOSSBlockOutputStream extends OutputStream {
    ...
    private final List<ListenableFuture<PartETagParam>> partETagsFutures;
    ...
    public synchronized void close() throws IOException {
        ...
        try {
            if (uploadId == null) {
                ...
                final List<PartETagParam> partETags = waitForAllPartUploads();
                if (null == partETags) {
    ...
    private void uploadCurrentBlock()
        ...
        OSSDataBlocks.BlockUploadData uploadData = dataBlock.startUpload();
        try {
            ListenableFuture<PartETagParam> partETagFuture =
                    executorService.submit(() -> {
                        try {
                            PartETagParam partETagParam = store.uploadPart(uploadData, size, key,
     ...
     */
    private List<PartETagParam> waitForAllPartUploads() throws IOException {
        ...
        try {
            ...
            for (ListenableFuture<PartETagParam> future : partETagsFutures) {
                future.cancel(true);
            }
