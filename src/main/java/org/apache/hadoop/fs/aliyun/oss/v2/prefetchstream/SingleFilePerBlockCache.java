...
//          prefetchingStatistics.blockRemovedFromFileCache();
    ...
    private static final String CACHE_FILE_PREFIX = "oss-fs-cache-";
    ...
    public static boolean isCacheSpaceAvailable(long fileSize, Configuration conf,
        ...
        try {
            Path cacheFilePath = getTempFilePath(conf, localDirAllocator, null);
    ...
    private static Path getTempFilePath(final Configuration conf,
                                        final LocalDirAllocator localDirAllocator, final String userDefinedPrefix) throws IOException {
        StringBuilder readPrefix = new StringBuilder().append(CACHE_FILE_PREFIX);
        if (userDefinedPrefix != null) {
            readPrefix.append(userDefinedPrefix);
            readPrefix.append("-");
        }
